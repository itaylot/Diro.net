...
.card-banner{height:215px;display:flex;align-items:center;justify-content:center;position:relative;flex-shrink:0;overflow:hidden}
/* subtle dot-grid texture so the empty banner looks intentional, not blank */
.card-banner::after{content:"";position:absolute;inset:0;background-image:radial-gradient(rgba(255,255,255,.07) 1px,transparent 1px);background-size:16px 16px;opacity:.6}
/* faint house watermark instead of a loud centered emoji */
.bn-ico{font-size:60px;opacity:.28;filter:drop-shadow(0 4px 14px rgba(0,0,0,.35));position:relative;z-index:1}
...
/* All source tags share one frosted-glass style — the label text already says
   the source, so no loud per-source colors needed (keeps the grid cohesive). */
.src-yad2,.src-fb,.src-hom{background:rgba(255,255,255,.92);color:#0D2B1A;border:1px solid rgba(255,255,255,.5)}
...
// On-brand placeholder for cards with no photo — earthy greens that match the
// header (not the source's brand color). Source is shown by the src-tag instead.
const BANS={Yad2:'linear-gradient(155deg,#10301d,#1f5236)',Homely:'linear-gradient(155deg,#0f3024,#176b3a)',Facebook:'linear-gradient(155deg,#0D2B1A,#1A4A2E)'};
const ban=s=>BANS[SK(s)]||'linear-gradient(155deg,#0D2B1A,#1A4A2E)';
